  ...
  login(){
    this.showLoading();
		...
		if(this.registerCredentials.username == "admin" && this.registerCredentials.password == "admin"){
			this.auth.login(this.registerCredentials, { 'data' : '{"logFlag": true,"modified": true,"reqType": 1,"siteId": 81}'}).subscribe(allowed => {
				if (allowed) {
					//AboutPage.reload();
					this.navCtrl.setRoot(TabsPage);
				} else {
			...
			this.http.get('https://mpljqoh0sc.execute-api.us-east-1.amazonaws.com/daoniFarmingDataLogger/login', 
			{ 
